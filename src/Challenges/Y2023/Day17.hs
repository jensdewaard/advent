...
import Common.Coord (Coord, Dir (..), cardinal, direction, moveN, reverse)
...
next (State c ds hs m) =
  [ (State c' (dirTaken : ds) (cost : hs) m, cost)
    | c' <- cardinal c,
...
nextB (StateB c ds hs m) =
  [ (StateB newP (replicate pathLength dirTaken ++ ds) (map fromJust pathHeats ++ hs) m, cost)
    | c' <- cardinal c,
      let dirTaken = direction c c',
      null ds || dirTaken /= reverse (head ds), -- do not reverse course
