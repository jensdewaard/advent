...
dropSand y c p =
  let p' = nextPosition c p
   in if p == p'
        then dropSand y ((p, 'o') : c) (500, 0) -- rests elsewhere
        else
          if snd p' > y
            then c
            else dropSand y c p'
...
dropSand' y c p =
  let p' = nextPosition c p
   in if p' == (500, 0)
        then (p', 'o') : c -- came to rest at source
        else
          if p == p'
            then dropSand' y ((p, 'o') : c) (500, 0) -- rests elsewhere
            else
              if snd p' > y
                then dropSand' y ((p', 'o') : c) (500, 0)
                else dropSand' y c p'

nextPosition :: Cave -> Coord -> Coord
nextPosition c p@(px, py) =
  let p1 = (px, py + 1)
      p2 = (px - 1, py + 1)
      p3 = (px + 1, py + 1)
   in if isNothing $ getF c p1
        then p1
        else
          if isNothing $ getF c p2
            then p2
            else if isNothing $ getF c p3 then p3 else p
