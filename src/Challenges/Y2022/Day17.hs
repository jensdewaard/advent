...
sim rs _ w@World {fallingRock = Nothing} = w {fallingRock = r, rockIndex = rockIndex w + 1}
  where
    r = Just $ rs !! rockIndex w $ (stackHeight w + 3)
sim _ ds w = w {fallingRock = mr, jetIndex = jetIndex w + 1, surface = stack', stackHeight = sh}
  where
